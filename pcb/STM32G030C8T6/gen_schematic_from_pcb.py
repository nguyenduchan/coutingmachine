...
STM32G030C8T6.kicad_sch is the source of truth. The PCB must match it
...
SYMLIB = ROOT / "libraries" / "STM32G030C8T6.kicad_sym"
...
def main() -> int:
    ...
    if "--from-pcb" not in sys.argv:
        ...
        print("PCB must match STM32G030C8T6.kicad_sch (verify_fab.py).")
    ...
    for c in comps:
        ...
        if sym is None:
            ...
            if sym not in used:
                ...
                body = body.replace(
                    f'(symbol "{sym}"', f'(symbol "STM32G030C8T6:{sym}"', 1
                )
                used[sym] = {"pins": pins,
            ...
            # instances (which reference "STM32G030C8T6:Name") match nothing and
    ...
    for c in comps:
        ...
        parts.append(
            f'\t(symbol (lib_id "STM32G030C8T6:{c["sym"]}") (at {ix} {iy} 0) (unit 1)\n'
