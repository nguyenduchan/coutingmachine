...
def _replace_uln_dip_with_module(pcb: str) -> str:
    ...
    for p in parts[1:]:
        ...
        labels = ["IN1", "IN2", "IN3", "IN4", "GND", "+12V"]
        lines = [
            f'\n\t(footprint "STM32G030C8T6:ULN2003_Module"',
            '\t\t(layer "F.Cu")',
            f'\t\t(uuid "{uid()}")',
            f"\t\t(at {x} {y} {rot})",
...
def patch_pcb() -> None:
    path = ROOT / "STM32G030C8T6.kicad_pcb"
...
def _update_uln_sch_symbols(sch: str) -> str:
    ...
    for ref in ("U5", "U6", "U7"):
        ...
        sch = re.sub(
            rf'(\(property "Reference" "{ref}"[\s\S]*?\(property "Footprint" ")STM32G030C8T6:ULN2003AN(")',
            rf"\1STM32G030C8T6:ULN2003_Module\2",
            sch,
            count=1,
        )
    return sch
...
def patch_sch() -> None:
    path = ROOT / "STM32G030C8T6.kicad_sch"
...
def main() -> None:
    ...
    patch_sch()
    # sanity
    sch = (ROOT / "STM32G030C8T6.kicad_sch").read_text(encoding="utf-8")
    pcb = (ROOT / "STM32G030C8T6.kicad_pcb").read_text(encoding="utf-8")
    for ref in ("J5", "J6", "J7"):
