#!/usr/bin/env python3
"""JLCPCB-ready pluggable sub-modules for STM32G030C8T6.
...
MOD = ROOT / "modules"
PRETTY = ROOT / "libraries" / "STM32G030C8T6.pretty"
PITCH = 2.54
...
def _hdr_male(a, ref: str, n: int, labels: list[str], x: float, y: float,
    ...
    span = (n - 1) * PITCH
    a(f'\t(footprint "STM32G030C8T6:PinHeader_1x{n:02d}_Module"')
    a('\t\t(layer "F.Cu")')
    a(f'\t\t(uuid "{uid()}")')
    a(f"\t\t(at {x} {y} {rot})")
...
def _ptc(a, ref: str, x: float, y: float, n1: tuple[int, str], n2: tuple[int, str]):
    ...
    a('\t(footprint "STM32G030C8T6:Fuse_PTC_Radial_5.1mm"')
    a('\t\t(layer "F.Cu")')
    a(f'\t\t(uuid "{uid()}")')
    a(f"\t\t(at {x} {y})")
    a(f'\t\t(property "Reference" "{ref}"')
...
def _fuse_5x20(
    ...
    a('\t(footprint "STM32G030C8T6:Fuse_Holder_5x20_Open"')
    a('\t\t(layer "F.Cu")')
    a(f'\t\t(uuid "{uid()}")')
    a(f"\t\t(at {x} {y} {rot})")
    a(f'\t\t(property "Reference" "{ref}"')
...
def gen_m3() -> Path:
    ...
    a('\t(footprint "STM32G030C8T6:ULN2003AN"')
    a('\t\t(layer "F.Cu")')
    a(f'\t\t(uuid "{uid()}")')
    a(f"\t\t(at {ux} {uy})")
    ...
    a('\t(footprint "STM32G030C8T6:JST_XH_05_BYJ"')
    a('\t\t(layer "F.Cu")')
    a(f'\t\t(uuid "{uid()}")')
    a(f"\t\t(at {jx} {jy})")
