...
| # | Yêu cầu | Script |
|---|---------|--------|
| B1 | GPIO chức năng khớp `s3_pinmap.py` (opto×4 + BYJ×12 + TMC + TFT LCD + buzzer + blower) | `verify_STM32G030C8T6_nets.py` |
| B2 | **Không** dùng IO35 / IO36 / IO37 (octal PSRAM N16R8) | `verify_STM32G030C8T6_nets.py` |
| B3 | IO0, IO19, IO20, TX0, RX0, RST **không** route | `verify_STM32G030C8T6_nets.py` |
| B4 | `OPTO_OUT1..8` trên U1 (IO1/2/4/5/7/8/14/15); IO9 = `BUZZER`; IO6 = `T_IRQ` | `verify_STM32G030C8T6_nets.py` |
...
| E11.8 | Connectivity / pinmap **PASS** trước route (`verify_connectivity.py`, `verify_STM32G030C8T6_nets.py`) | `verify_all.py` phần net |
...
- [ ] Mở `STM32G030C8T6.kicad_pcb` — F.Cu + B.Cu; kiểm bus TFT / motor / shift
...
python verify_STM32G030C8T6_nets.py
