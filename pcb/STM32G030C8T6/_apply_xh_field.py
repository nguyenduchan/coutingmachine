...
def patch_carrier(t: str) -> str:
    ...
    a(f'\\t(footprint "STM32G030C8T6:{OPTO4_FP}"')
    a('\\t\\t(layer "F.Cu")')
    a(f'\\t\\t(uuid "{uid()}")')
    a(f"\\t\\t(at {j31x} {j31y})")
    ...
    t = _replace_once(
        t,
        '    a(\'\\t(footprint "STM32G030C8T6:PinHeader_1x03_Buzzer"\'',
    ...
    t = _replace_once(
        t,
        '    a(\'\\t(footprint "STM32G030C8T6:PinHeader_1x04_MOSFET"\'',
...
def patch_submodules(t: str) -> str:
    ...
    n = len(labels)
    span = (n - 1) * XH_PITCH
    a(f'\\t(footprint "STM32G030C8T6:JST_XH_04_Plug"')
    a('\\t\\t(layer "F.Cu")')
    a(f'\\t\\t(uuid "{uid()}")')
    a(f"\\t\\t(at {x} {y})")
    a(f'\\t\\t(property "Reference" "{ref}"')
    ...
    n = len(labels)
    span = (n - 1) * XH_PITCH
    a(f'\\t(footprint "STM32G030C8T6:JST_XH_{n:02d}_Plug"')
    a('\\t\\t(layer "F.Cu")')
    a(f'\\t\\t(uuid "{uid()}")')
    a(f"\\t\\t(at {x} {y})")
    a(f'\\t\\t(property "Reference" "{ref}"')
