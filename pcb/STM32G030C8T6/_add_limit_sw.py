...
def main() -> None:
    ...
    for mi, (ref_u, ref_j, jmin, jmax, xu, yu, xj, yj, pin_in1, pin_in2, u4min, u4max, lab) in enumerate(l298n_place):'''
        ...
        for jref, xjl, u4p, tag in [
            ...
            psw = (xjl, yj + 1.27)
            parts += [
                f'\\t(symbol (lib_id "STM32G030C8T6:Conn_1x02_LimitSW") (at {xjl} {yj} 0) (unit 1)',
                f'\\t\\t(uuid "{ju}")',
                ...
                "\\t\\t\\t(effects (font (size 1.27 1.27)))",
                "\\t\\t)",
                f'\\t\\t(property "Footprint" "STM32G030C8T6:PinHeader_1x02_LimitSW" (at {xjl} {yj} 0)',
                ...
                f'\\t\\t(pin "2" (uuid "{uid()}"))',
                "\\t\\t(instances",
                f'\\t\\t\\t(project "STM32G030C8T6" (path "/{sch_uuid}" (reference "{jref}") (unit 1)))',
                "\\t\\t)",
                "\\t)",
            ]
...
'''
    ...
    if "TRUC1 MOT+LIM" not in t and "J8" not in t[t.find("l298n_pcb") : t.find("l298n_pcb") + 800]:
        ...
        a('\\t(footprint "STM32G030C8T6:PinHeader_1x02_MotorDC"')'''
