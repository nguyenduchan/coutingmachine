# Rotary_Linear sim — spur rack & pinion (exact FreeCAD meshes)

## Active CAD parts (print / bolt)
| Part | Role |
|------|------|
| `RL_Pinion_Shaft` | Pinion + shaft **fused** (one 3D print) |
| `RL_Bearing_Rail_S/N` | Lower saddle + rail; bolt to base (M3) |
| `RL_Bearing_Cap_S/N` | Upper half; **M3** clamp onto journals |
| `RL_Knob` | Drive; **M3 cross-bolt** onto shaft end |
| `RL_Friction_Washer` | Friction hold |
| `RL_Follower` | Rack + Z slide |
...
2. Drop `RL_Pinion_Shaft` into open saddles (mesh rack).
