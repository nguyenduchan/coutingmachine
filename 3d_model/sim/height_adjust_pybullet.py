"""
Height_Adjust physics — EXACT FreeCAD meshes (STL) in PyBullet.
...
MESH = ROOT / "meshes" / "height_adjust"
OUT = ROOT / "out"
OUT.mkdir(exist_ok=True)
METRICS = OUT / "height_adjust_metrics.json"
REVIEW = OUT / "height_adjust_physics_review.txt"
...
def ensure_meshes() -> dict:
    ...
    if not man.is_file() or not (MESH / "HA_Pinion_Shaft.stl").is_file():
        raise SystemExit(
...
def axis_pivot_from_manifest(manifest: dict) -> list[float]:
    ...
    pin = (manifest.get("parts") or {}).get("HA_Pinion_Shaft") or {}
...
def make_follower_collision(cid, manifest: dict) -> int:
    ...
    pin = (manifest.get("parts") or {}).get("HA_Pinion_Shaft", {}).get("bbox_mm") or {}
    ...
    fol = (manifest.get("parts") or {}).get("HA_Follower", {}).get("bbox_mm") or {}
...
def build(cid, manifest: dict, follower_col: Path) -> dict:
    ids: dict = {}
    rgba = {
        "HA_Pinion_Shaft": [1.0, 0.45, 0.05, 1],
        "HA_Knob": [0.45, 0.25, 0.55, 1],
        "HA_Follower": [0.25, 0.72, 0.35, 1],
        "HA_Bearing_Rail_S": [0.30, 0.50, 0.75, 1],
        "HA_Bearing_Cap_S": [0.40, 0.60, 0.85, 1],
        "HA_Bearing_Rail_N": [0.30, 0.50, 0.75, 1],
        "HA_Bearing_Cap_N": [0.40, 0.60, 0.85, 1],
        "HA_Friction_Washer": [0.35, 0.35, 0.38, 1],
    }
    ...
    for name in manifest["roles"]["static"]:
        ...
        if name.startswith("HA_Guide_Rail"):
    ...
    fol_stl = MESH / "HA_Follower.stl"
    col = make_follower_collision(cid, manifest)
    vis = p.createVisualShape(
        p.GEOM_MESH,
        fileName=str(fol_stl.resolve()),
        meshScale=[S, S, S],
        rgbaColor=rgba["HA_Follower"],
        physicsClientId=cid,
    )
    ids["HA_Follower"] = p.createMultiBody(
    ...
    for name in ("HA_Friction_Washer",):
        if name in ids:
            p.setCollisionFilterPair(
                ids[name], ids["HA_Follower"], -1, -1, 0, physicsClientId=cid
            )
    ...
    ids["pinion"] = ids.get("HA_Pinion_Shaft")
    ids["follower"] = ids["HA_Follower"]
...
def run(gui: bool, seconds: float):
    ...
    cid = world(gui)
    ids = build(cid, manifest, MESH / "HA_Follower.stl")
    ...
    lines = [
        ...
        "ONE actuator = HA_Pinion(+shaft/knob) kinematic | HA_Follower(+rack) dynamic",
    ...
    report = {
        ...
        "force_sources": ["HA_Pinion_assembly_kinematic_about_Y"],
